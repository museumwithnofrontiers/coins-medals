...
export default {
  ...
  // addresses every DXA config repeats, Coins and Medals adding none of its
  // own.
  links: { ...mwnfLinks },

  // The source-database chip's colour, per project (epic #1727 phase 4):
  // one entry for every project id `npm pack @museumwnf/coins-medals-data`
  // carries in `manifest.projects` (checked 2026-09-21, coins-medals-data
  ...
  // (`manifest.projects[id].name.en`). This gallery owns no project of its
  // own — every member is borrowed, same as amulets.
  projectColors: {
  ...
  // Explore Islamic Art Collections project; Coins and Medals, itself a
  // borrower of that project, follows the same choice.
  ...
  site: { origin: 'https://museumwithnofrontiers.github.io/coins-medals' },
  ...
  extraViews: [
    ...
    // results/gallery) is byte-identical to carpets' and amulets' own on
    // `origin/main` (epic inventory-app#1731) — the package's shared gallery
    // pages, spread in with this site's one genuine per-site string
    // (`Credits.vue`'s `body` entry name).
    ...standardRoutes('gallery', { creditsBody: 'coins.credits.body' }),
  ],
