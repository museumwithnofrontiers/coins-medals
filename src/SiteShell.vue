...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('coins.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('coins.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('coins.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
