...
describe('website smoke test', () => {
  it('mounts against the configured data package', async () => {
    ...
    expect(host.textContent).toContain('Coins and Medals')
  ...
  it('renders a keyword search on the composed results view', async () => {
    ...
    // stripped of Markdown emphasis characters, which this package's own
    // italicised names carry (e.g. "*Dinar*") and the query syntax does not.
    const term = items[0].internal_name.replace(/\*/g, '').split(' ')[0]
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    // A phrase from the extracted `gallery.about.body` (locales/en.json).
    expect(host.textContent).toContain('We suggest you start your visit in the')
  ...
  it('renders the item sheet on the composed record view', async () => {
    ...
    // (`useProjects().label()`), not a legacy project-code badge — this
  ...
  // off a legacy project code. Sharing History is this package's dominant
  // project by item count (56 of 100); a Discover Islamic Art item exercises
  // a project other than that dominant one.
  it('colours and names the source-database chip from the manifest projects section', async () => {
    ...
    // `backward_compatibility` code is no longer rendered. This item is the
    // one in the package whose `related_items` carries an `in_package: false`
    // entry.
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'd3cd72eb-4da3-52ee-8fe8-e450b2d00a90')
    const outsideReference = item.related_items.find((ref) => ref.in_package === false)
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    ...
    const isl = await mountSite('#/item/d3cd72eb-4da3-52ee-8fe8-e450b2d00a90')
  ...
  // for Discover Islamic Art, the borrowed ISL record's project, so this
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    // Stripped of Markdown emphasis and punctuation the boolean query syntax
    // does not accept — the same reasoning as the keyword-search test above.
    const term = item.internal_name.replace(/[*().]/g, '').trim()
    const { app, host } = await mountSite(`#/search?q=${encodeURIComponent(term)}`)
    await vi.waitFor(() => expect(host.querySelector('.mwnf-grid__tile')).not.toBeNull(), { timeout: 20000 })
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    // An Explore Islamic Art Collections coin with the Umayyads dynasty
    // attached — a record whose dynasty translation actually carries a
    // history, so it survives ItemSheet.vue's own "has something to show"
    // filter. The same item as the notice-positive case above.
    const { app, host } = await mountSite('#/item/c390c585-bede-5ad5-9758-01796227dd08')
  ...
  it('renders the timeline results on the composed timeline view', async () => {
    ...
    expect(host.querySelector('.mwnf-summary').textContent).toContain('14')
    // A Lebanon event's own description, not just a row count — its
    // translation loads asynchronously, behind the spec's own `tr`.
    await vi.waitFor(() => expect(host.textContent).toContain('Jamal Pasha initiates a blockade'), { timeout: 20000 })
    // The join to member items finds Lebanon's five dated objects even with
  ...
  it('renders the timeline gallery on the composed results view', async () => {
    const { app, host } = await mountSite('#/timeline/gallery?country=lb')
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-grid__tile').length).toBe(5), { timeout: 20000 })
    expect(host.querySelector('.mwnf-summary').textContent).toContain('Lebanon')
    expect(host.textContent).toContain('25 Syrian Piastres')
    app.unmount()
  }, 60000)

  // metanull/carpets#42: the composed entrance's own country control writes
  // the inventory id (`lbn`), not the legacy two-letter code (`lb`) the
  // gallery's own deep links carry — `countryIdForCode` used to answer
  // nothing for an id, so every event matched instead of Lebanon's own. Both
  ...
  it('filters the timeline the same way on the inventory id as on the legacy code', async () => {
    const { app, host } = await mountSite('#/timeline-results?country=lbn')
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-timeline__row').length).toBe(14), { timeout: 20000 })
    expect(host.querySelector('.mwnf-summary').textContent).toContain('14')
    await vi.waitFor(() => expect(host.textContent).toContain('Jamal Pasha initiates a blockade'), { timeout: 20000 })
    expect(host.querySelector('.mwnf-timeline__gallery').textContent).toContain('5')
  ...
  it("submits the timeline entrance's country control (an id) to a results address that actually filters", async () => {
    ...
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-timeline__row').length).toBe(14), { timeout: 20000 })
    expect(host.querySelector('.mwnf-timeline__caption').textContent).toContain('Lebanon')
    app.unmount()
  }, 60000)
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('Kunsthistorisches Museum')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/a3753de5-842a-5f0b-a8c7-50bb2e312ce6')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Kunsthistorisches Museum'), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/a3753de5-842a-5f0b-a8c7-50bb2e312ce6/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['coins', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
